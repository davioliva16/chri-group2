...
class PA:
    ...
    def run(self):
        ...
        pos_endpoint = xh
        pos_endpoint_last_frame = self.xh_last_frame
        delta_time = 1 #assuming delta_time to equal 1 frame

        #for spring 
        distance_from_center = pos_center - pos_endpoint

        #for damping
        velocity = -(pos_endpoint - pos_endpoint_last_frame) / delta_time

        f_spring = -self.k * distance_from_center
        f_damp = -self.b * velocity
