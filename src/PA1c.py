...
class PA:
    def __init__(self):
        ...
        self.unhandledFreeSpace = False #Bool to when we leave the wall
        self.sideCollision = False
        self.hideOriginalObject = False
        ##############################################

    def god_object(self, xh, wall: pygame.Rect, kc):
        g = self.graphics

        inCollision = g.haptic.colliderect(wall)
        
        if inCollision:
        
            if self.unhandledCollision:
                if (xh[0]+24 - wall.left) < (xh[1]+24 - wall.top): #Side collision:
                    self.sideCollision = True
                else:
                    self.sideCollision = False
                
                if self.hideOriginalObject:
                    g.haptic.inflate_ip(-47,-47)
                self.unhandledCollision = False

            #Figure out which coordinate was closests
            if self.sideCollision:
                proxyPosition = np.array([wall.left-24, xh[1]])
            else:
                proxyPosition = np.array([xh[0], wall.top-24])
            ...
            pygame.draw.rect(g.screenVR, g.cRed, proxyRectangle)
            ...
            self.unhandledCollision = True
            f_collision = np.array([0,0])

        return f_collision, inCollision
    ...
    def run(self):
        ...
        #draw in sim
        pygame.draw.rect(g.screenVR, g.cDarkblue, wall)
        pygame.draw.rect(g.screenHaptics, g.cDarkblue, wall)
        ...
        for key in keyups:
            ...
            if key == ord('t'): #Toggle hiding feature
                self.hideOriginalObject = not self.hideOriginalObject 
            #you can add more if statements to handle additional key characters

        f_god_object, inCollison = self.god_object(xh, wall, self.kc)
