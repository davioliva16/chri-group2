...
class game:
    def __init__(self):
        ...
        #TODO Maybe figure out how to full screen
        ...
        #Compute Height Map and save it to a surface varible
        self.gradient_surface = pygame.Surface( self.graphics.window_size)
        '''
        for x in range(0,600):
            intensity = round(255/2 * self.height_value(pos_x=x) + 255/2)
            pygame.draw.lines(self.gradient_surface, (intensity,intensity,intensity), False,[(x,0),(x,400)],1)
        '''
        ##############################################

    '''
    def height_value(self,pos_x):
        return math.cos(pos_x/(20*math.pi) + 300)
        
    def height_gradient(self,pos_x):
        return -math.sin(pos_x/(20*math.pi) + 300)/(20*math.pi)
    '''
    ...
    def run(self):
        ...
        #Draw Pre-Computed Height Map
        g.screenHaptics.blit(self.gradient_surface, (0, 0))
        '''
        f_hill = self.k*self.height_gradient(xh[0])
        '''
        fe = np.array([0.0, 0.0])
